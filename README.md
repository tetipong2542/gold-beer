...
|-----------|-------------|
| `[gold_price]` | แสดงราคาเต็ม (ทองแท่ง + รูปพรรณ + การเปลี่ยนแปลง) |
...
เพิ่ม header และ/หรือ footer ให้กับ type="bar" หรือ type="ornament"

| Shortcode | Description |
|-----------|-------------|
| `[gold_price type="bar" show="header"]` | ทองแท่ง + header (ชื่อ + เวลาอัพเดท) |
| `[gold_price type="bar" show="footer"]` | ทองแท่ง + footer (วันนี้, ล่าสุด, วันที่) |
| `[gold_price type="bar" show="header,footer"]` | ทองแท่ง + header + footer |
| `[gold_price type="ornament" show="header,footer"]` | ทองรูปพรรณ + header + footer |

**show values:**
- `header` - 🏆 ราคาทองวันนี้ + เวลาอัพเดท
- `footer` - วันนี้ (+1,950), ล่าสุด (-150), ครั้งที่, วันที่เวลา

#### hide Parameter

ซ่อนส่วนที่ไม่ต้องการแสดง

| Shortcode | Description |
|-----------|-------------|
| `[gold_price hide="gpd-header"]` | ซ่อน header |
| `[gold_price hide="gpd-footer"]` | ซ่อน footer |
| `[gold_price hide="gpd-datetime"]` | ซ่อนวันที่เวลา |
| `[gold_price hide="gpd-header,gpd-datetime"]` | ซ่อนหลายส่วน |

**hide values (CSS classes):**
- `gpd-header` - Header ทั้งหมด
- `gpd-footer` - Footer ทั้งหมด
- `gpd-grid` - Grid ราคา
- `gpd-datetime` - วันที่เวลา
- `gpd-change-row` - แถวการเปลี่ยนแปลง
- `gpd-card` - Card แต่ละอัน

#### ใช้ show และ hide ร่วมกัน

```
[gold_price type="bar" show="header,footer" hide="gpd-datetime"]
```
→ แสดงทองแท่ง + header + footer แต่ซ่อนวันที่เวลา
